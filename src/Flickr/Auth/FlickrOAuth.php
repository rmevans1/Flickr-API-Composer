<?php
	...
	public function getToken()
	{
		...
		for($i = 0; $i<count($level1); $i++)
		{
			...
			$keypair = explode("=",$level1[$i]);
			$level2[$keypair[0]] = $keypair[1];
		}
		echo "<pre>";
		print_r($level2);
		echo "</pre>";
    ...
    public function setURL ($path)
	{
        if (empty($path))
		{
            throw new FlickrOAuthException('No path specified for OAuthSimple.setURL');
		}
        $this->_path=$path;
    ...
    public function setAction ($action) 
    {
		...
		{
            throw new FlickrOAuthException('Invalid action specified for OAuthSimple.setAction');
		}
        $this->_action = $action;
    ...
    public function signatures ($signatures) 
    {
		...
		{
            throw new FlickrOAuthException('Must pass dictionary array to OAuthSimple.signatures');
		}
        if (!empty($signatures))
        {
		...
		{
            throw new FlickrOAuthException('Missing required consumer_key in OAuthSimple.signatures');
        }
        if (empty($this->_secrets['shared_secret']))
		{
            throw new FlickrOAuthException('Missing requires shared_secret in OAuthSimple.signatures');
		}
        if (!empty($this->_secrets['oauth_token']) && empty($this->_secrets['oauth_secret']))
		{
            throw new FlickrOAuthException('Missing oauth_secret for supplied oauth_token in OAuthSimple.signatures');
		}
    ...
    public function setSignatureMethod ($method="")
	{
        ...
        switch($method)
        {
            ...
            case 'HMAC-SHA1':
				...
				break;
            default:
                throw new FlickrOAuthException("Unknown signing method $method specified for OAuthSimple.setSignatureMethod");
				break;
        }
    ...
    */
    ...
    private static function _oauthEscape($string) 
    {
        ...
        if (is_array($string)) {
            throw new FlickrOAuthException('Array passed to _oauthEscape');
		}
    ...
    private function _getApiKey() 
    {
        if (empty($this->_secrets['consumer_key']))
        {
            throw new FlickrOAuthException('No consumer_key set for OAuthSimple');
        }
    ...
    private function _getAccessToken() 
    {
		...
		{
            throw new FlickrOAuthException('No access token (oauth_token) set for OAuthSimple.');
		}
    ...
    private function _generateSignature () 
    {
        ...
        switch($this->_parameters['oauth_signature_method'])
        {
            ...
            default:
                throw new FlickrOAuthException('Unknown signature method for OAuthSimple');
				break;
        }
    }
}
